...
const TEAM = [
  { name: "Sohaib Aref",    role: "Co-Founder & CD" },
  { name: "Osama Nabil",    role: "Co-Founder & CEO" },
  { name: "Yosuf Awad",     role: "Public Relations" },
  { name: "Osama Tala",     role: "Marketing strategies" },
  { name: "Rslan Alkendi",  role: "Creative" },
  { name: "Dia Osaimi",     role: "IT specialist" },
  { name: "Moaz El Mahdi",  role: "Full Stack Developer (SoftEng)" },
  { name: "Omima Amar",     role: "Creative" },
  { name: "Amr khulaidi",   role: "Creative" },
  { name: "Reem Al Matary", role: "Creative" },
];

export default function Team() {
  const trackRef = useRef(null);

  // نقيس العرض الكلي للتراك (المكرر مرتين) ونحسب نصفه بالبيكسل
  useEffect(() => {
    ...
    const measure = () => {
      ...
      const half = Math.max(0, Math.round(total / 2));
      el.style.setProperty("--half", half + "px"); // نصف العرض
    };

    measure();
    const t = setTimeout(measure, 50); // احتياط لوقت تحميل الصور
      ...
      <div className="team-wrap">
        ...
        <div className="team-viewport">
          <div ref={trackRef} className="team-track">
            {TEAM.map((m, i) => <Card key={`a-${i}`} {...m} />)}
            {TEAM.map((m, i) => <Card key={`b-${i}`} {...m} />)}
          </div>
        </div>
      </div>
    </section>
  ...
  return (
    <div className="team-card">
      <div className="img-wrap">
        <img src={moazImg} alt={name} />
        <div className="social-icons">
          <a href={LINKEDIN} target="_blank" rel="noreferrer" aria-label="LinkedIn" className="icon-btn" title="LinkedIn">
          ...
          <a href={INSTAGRAM} target="_blank" rel="noreferrer" aria-label="Instagram" className="icon-btn" title="Instagram">
